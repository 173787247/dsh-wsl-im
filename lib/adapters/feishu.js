...
/** Group chats: skip when Feishu did not include a mention (OryxOS drops unmentioned group msgs). */
...
export function createFeishuAdapter({ appId, appSecret, onMessage, logger }) {
  ...
  return {
    ...
    async start() {
      ...
      state = "up";
      logger?.info?.(
        `dsh-wsl-im feishu WS client started (OryxOS-aligned long connection, ${proxyLabel()})`,
      );
    },
    stop() {
      state = "down";
