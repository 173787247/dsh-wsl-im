 ...
 * DingTalk Stream wire protocol.
 *   POST https://api.dingtalk.com/v1.0/gateway/connections/open
 *   topic /v1.0/im/bot/messages/get → sessionWebhook reply
 */

import WebSocket from "ws";
import { proxiedFetch, resolveWsProxyAgent } from "../proxy.js";
...
export function createDingtalkAdapter({ clientId, clientSecret, onMessage, logger }) {
  ...
  async function openConnection() {
    const res = await proxiedFetch(`${API}/v1.0/gateway/connections/open`, {
      method: "POST",
  ...
  async function handleFrame(raw) {
    ...
    if (type === "CALLBACK" && topic === TOPIC_BOT) {
      // ACK the stream frame before handling the bot payload.
      ws.send(
        JSON.stringify({
          code: 200,
          headers: frame.headers,
      ...
      const reply = async (replyText) => {
        ...
        await proxiedFetch(sessionWebhook, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            msgtype: "text",
  ...
  return {
    ...
    async start() {
      stopped = false;
      await connect();
      logger?.info?.("dsh-wsl-im dingtalk Stream connected");
    },
    stop() {
      stopped = true;
      state = "down";
