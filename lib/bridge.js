...
export class Bridge {
  ...
  async createChat(key) {
    ...
    const cwd = this.resolveAgentCwd();
  ...
  resolveAgentCwd() {
    const configured = this.config.agent.cwd;
    if (configured) return configured;
    // Prefer a stable WSL home workspace — /mnt/c link cwd lacks tools context and is slow.
    const dir = join(homedir(), ".dsh", "im-workspace");
    try {
  ...
  async runTurn(chat, input) {
    ...
    try {
      ...
      if (files.length) {
        ...
        const inbox = join(chat.cwd || this.resolveAgentCwd(), "inbox");
        try {
